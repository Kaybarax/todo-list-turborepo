...
const createTodo = async todo => {
  ...
  await waitFor(element(by.id(`todo-item-${todo.title}`)))
    .toBeVisible()
    .withTimeout(3000);
};
...
describe('Mobile Todo Management E2E Tests', () => {
  ...
  describe('Todo CRUD Operations', () => {
    ...
    it('should edit an existing todo', async () => {
      ...
      await expect(element(by.id('todo-title')).withAncestor(by.id(`todo-item-${updatedTitle}`))).toHaveText(
        updatedTitle,
      );
    });
  ...
  describe('Todo List Management', () => {
    ...
    it('should sort todos by priority', async () => {
      ...
      await expect(
        element(by.id(`todo-item-${testTodos[0].title}`))
          .withAncestor(todoList)
          .atIndex(0),
      ).toBeVisible(); // high
      await expect(
        element(by.id(`todo-item-${testTodos[1].title}`))
          .withAncestor(todoList)
          .atIndex(1),
      ).toBeVisible(); // medium
      await expect(
        element(by.id(`todo-item-${testTodos[2].title}`))
          .withAncestor(todoList)
          .atIndex(2),
      ).toBeVisible(); // low
    });
  ...
  describe('Wallet Integration', () => {
    ...
    it('should create blockchain todo', async () => {
      ...
      await waitFor(element(by.id('transaction-confirmed')))
        .toBeVisible()
        .withTimeout(5000);
