...
async function checkUpstream(
  ...
  try {
    ...
    const errorLabel = error instanceof DOMException && error.name === 'AbortError' ? 'timeout' : 'unreachable';
    return {
      id,
      serviceName,
      healthy: false,
...
export async function checkGatewayHealth(): Promise<GatewayHealth> {
  ...
  const checks = await Promise.all(entries.map(u => checkUpstream(u.id, u.baseUrl, u.healthPath, u.serviceName)));
