...
async function main() {
  ...
  const idl = JSON.parse(fs.readFileSync(path.join(__dirname, '../target/idl/todo_program.json'), 'utf-8'));

  const program = new anchor.Program(idl, programId, provider) as unknown as Program<TodoProgram>;
  ...
  const createTx = await program.methods
    .createTodo('Complete Solana integration', 'Implement the Solana todo program for the monorepo modernization', {
      high: {},
    })
    .accounts({
      todoList: todoListPda,
