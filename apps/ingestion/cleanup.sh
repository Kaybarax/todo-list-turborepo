...
usage(){ echo "Usage: apps/ingestion/cleanup.sh [--yes] [--dry-run] [--verbose]"; }
...
cd "$(cd "$(dirname "$0")" && pwd)"
echo "Cleaning Ingestion build artifacts..."
...
if confirm "Remove node_modules in apps/ingestion?"; then run_cmd "rm -rf node_modules"; fi
echo "Ingestion cleanup done."
