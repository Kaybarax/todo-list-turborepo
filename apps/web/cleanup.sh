...
usage(){ echo "Usage: apps/web/cleanup.sh [--yes] [--dry-run] [--verbose]"; }
...
dirs=(.next dist build coverage logs .turbo .cache out storybook-static chromatic-output)
...
if confirm "Remove node_modules in apps/web?"; then run_cmd "rm -rf node_modules"; fi
echo "Web cleanup done."
