...
module.exports = results => {
  ...
  const processedResults = {
    ...
    customSummary: {
      ...
      integrationScenarios: {
        crudOperations: {
          moonbeam: results.testResults.filter(
            r =>
          ...
          base: results.testResults.filter(
            r =>
              r.testFilePath.includes('blockchain-expansion-integration') &&
      ...
      performance: {
        averageTestDuration:
          results.testResults.reduce((acc, result) => {
  ...
  console.log(
    `    - Moonbeam: ${processedResults.customSummary.integrationScenarios.crudOperations.moonbeam} test suites`,
  );
  console.log(`    - Base: ${processedResults.customSummary.integrationScenarios.crudOperations.base} test suites`);
  console.log(
    `  Network Switching: ${processedResults.customSummary.integrationScenarios.networkSwitching} test suites`,
  );
  console.log(
    `  Cross-Network Consistency: ${processedResults.customSummary.integrationScenarios.crossNetworkConsistency} test suites`,
  );

  if (processedResults.customSummary.performance.averageTestDuration > 0) {
    console.log(`\n⚡ Performance:`);
    console.log(
      `  Average Test Duration: ${Math.round(processedResults.customSummary.performance.averageTestDuration)}ms`,
    );
