...
usage(){ echo "Usage: packages/config-release/cleanup.sh [--yes] [--dry-run] [--verbose]"; }
...
cd "$(cd "$(dirname "$0")" && pwd)"
echo "Cleaning packages/config-release artifacts..."
dirs=(dist build .turbo .cache coverage logs)
files=(.eslintcache tsconfig.tsbuildinfo)
for d in "${dirs[@]}"; do [[ -d $d ]] && run_cmd "rm -rf $d"; done
for f in "${files[@]}"; do [[ -f $f ]] && run_cmd "rm -f $f"; done
if confirm "Remove node_modules in packages/config-release?"; then run_cmd "rm -rf node_modules"; fi
echo "packages/config-release cleanup done."
