...
usage(){ echo "Usage: packages/utils/cleanup.sh [--yes] [--dry-run] [--verbose]"; }
...
cd "$(cd "$(dirname "$0")" && pwd)"
echo "Cleaning packages/utils artifacts..."
...
if confirm "Remove node_modules in packages/utils?"; then run_cmd "rm -rf node_modules"; fi
echo "packages/utils cleanup done."
