...
usage(){ echo "Usage: packages/config-jest/cleanup.sh [--yes] [--dry-run] [--verbose]"; }
...
cd "$(cd "$(dirname "$0")" && pwd)"
echo "Cleaning packages/config-jest artifacts..."
...
if confirm "Remove node_modules in packages/config-jest?"; then run_cmd "rm -rf node_modules"; fi
echo "packages/config-jest cleanup done."
