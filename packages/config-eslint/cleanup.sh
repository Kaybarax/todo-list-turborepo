...
usage(){ echo "Usage: packages/config-eslint/cleanup.sh [--yes] [--dry-run] [--verbose]"; }
...
cd "$(cd "$(dirname "$0")" && pwd)"
echo "Cleaning packages/config-eslint artifacts..."
...
if confirm "Remove node_modules in packages/config-eslint?"; then run_cmd "rm -rf node_modules"; fi
echo "packages/config-eslint cleanup done."
