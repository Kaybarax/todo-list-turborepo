...
export class MoonbeamBlockchainService extends BaseBlockchainService {
  ...
  async connectWallet(_provider: any): Promise<WalletInfo> {
    try {
      ...
      throw BlockchainError.moonbeamConnectionFailed(
        'Failed to connect to Moonbeam wallet',
        error,
        this.network
      );
    }
  ...
  async getWalletBalance(tokenAddress?: string): Promise<string> {
    ...
    try {
      ...
      throw BlockchainError.networkError(
        'Failed to get wallet balance on Moonbeam',
        error,
        this.network
      );
    }
  ...
  async getTodos(): Promise<BlockchainTodo[]> {
    ...
    try {
      ...
      throw BlockchainError.contractError(
        'Failed to get todos from Moonbeam',
        error,
        this.network
      );
    }
  ...
  async getTodoById(id: string): Promise<BlockchainTodo | null> {
    ...
    try {
      ...
      throw BlockchainError.contractError(
        `Failed to get todo with ID ${id} from Moonbeam`,
        error,
        this.network
      );
    }
    ...
    try {
      ...
      if (this.isMoonbeamSpecificError(error)) {
        throw BlockchainError.moonbeamEvmError(
          'Failed to create todo on Moonbeam EVM',
          error,
          this.network
        );
      }
      throw BlockchainError.transactionFailed(
        'Failed to create todo on Moonbeam',
        undefined,
        error,
        this.network
      );
  ...
  private async initializeContracts(): Promise<void> {
    try {
      ...
      throw BlockchainError.contractError(
        'Failed to initialize contracts on Moonbeam',
        error,
        this.network
      );
    }
