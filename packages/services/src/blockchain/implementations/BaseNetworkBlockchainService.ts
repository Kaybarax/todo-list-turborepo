...
export class BaseNetworkBlockchainService extends BaseBlockchainService {
  ...
  async connectWallet(provider: any): Promise<WalletInfo> {
    try {
      ...
      throw BlockchainError.baseL2Error(
        'Failed to connect to Base wallet',
        error,
        this.network
      );
    }
  ...
  async getWalletBalance(tokenAddress?: string): Promise<string> {
    ...
    try {
      ...
      throw BlockchainError.networkError(
        'Failed to get wallet balance on Base',
        error,
        this.network
      );
    }
  ...
  async getTodos(): Promise<BlockchainTodo[]> {
    ...
    try {
      ...
      throw BlockchainError.contractError(
        'Failed to get todos from Base',
        error,
        this.network
      );
    }
  ...
  async getTodoById(id: string): Promise<BlockchainTodo | null> {
    ...
    try {
      ...
      throw BlockchainError.contractError(
        `Failed to get todo with ID ${id} from Base`,
        error,
        this.network
      );
    }
  ...
  async createTodo(todo: CreateBlockchainTodoInput): Promise<TransactionReceipt> {
    ...
    try {
      ...
      if (this.isBaseSpecificError(error)) {
        throw BlockchainError.baseL2Error(
          'Failed to create todo on Base L2',
          error,
          this.network
        );
      }
      throw BlockchainError.transactionFailed(
        'Failed to create todo on Base',
        undefined,
        error,
        this.network
      );
  ...
  async updateTodo(id: string, todo: UpdateBlockchainTodoInput): Promise<TransactionReceipt> {
    ...
    try {
      ...
      if (this.isBaseSpecificError(error)) {
        throw BlockchainError.baseL2Error(
          `Failed to update todo with ID ${id} on Base L2`,
          error,
          this.network
        );
      }
  ...
  async deleteTodo(id: string): Promise<TransactionReceipt> {
    ...
    try {
      ...
      if (this.isBaseSpecificError(error)) {
        throw BlockchainError.baseL2Error(
          `Failed to delete todo with ID ${id} on Base L2`,
          error,
          this.network
        );
      }
  ...
  async getTransactionStatus(txHash: string): Promise<TransactionStatus> {
    try {
      ...
      throw BlockchainError.networkError(
        `Failed to get transaction status for ${txHash} on Base`,
        error,
        this.network
      );
    }
  ...
  private async initializeContracts(): Promise<void> {
    try {
      ...
      throw BlockchainError.contractError(
        'Failed to initialize contracts on Base',
        error,
        this.network
      );
    }
