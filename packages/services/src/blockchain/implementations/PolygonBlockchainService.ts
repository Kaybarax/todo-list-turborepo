...
export class PolygonBlockchainService extends BaseBlockchainService {
  ...
  async connectWallet(provider: any): Promise<WalletInfo> {
    try {
      ...
      throw BlockchainError.walletConnectionFailed(
        'Failed to connect to Polygon wallet',
        error,
        this.network
      );
    }
  ...
  async getTodos(): Promise<BlockchainTodo[]> {
    ...
    try {
      ...
      throw BlockchainError.contractError(
        'Failed to get todos from Polygon',
        error,
        this.network
      );
    }
  ...
  async getTodoById(id: string): Promise<BlockchainTodo | null> {
    ...
    try {
      ...
      throw BlockchainError.contractError(
        `Failed to get todo with ID ${id} from Polygon`,
        error,
        this.network
      );
    }
  ...
  async createTodo(todo: CreateBlockchainTodoInput): Promise<TransactionReceipt> {
    ...
    try {
      ...
      throw BlockchainError.transactionFailed(
        'Failed to create todo on Polygon',
        undefined,
        error,
        this.network
      );
