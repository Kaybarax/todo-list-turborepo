...
import { apiTodoSchema, createApiTodoSchema, updateApiTodoSchema } from './types';
...
export class TodoApiClient extends BaseApiClient {
    ...
    async getTodos(params) {
        try {
            const response = await this.get('/todos', { params });
    ...
    async createTodo(todo) {
        try {
            ...
            const response = await this.post('/todos', result.data);
            // Validate response data
            if (response.success && response.data) {
    ...
    async updateTodo(id, todo) {
        try {
            ...
            const response = await this.put(`/todos/${id}`, result.data);
            // Validate response data
            if (response.success && response.data) {
                const todoResult = apiTodoSchema.safeParse(response.data);
                if (!todoResult.success) {
    ...
    async toggleTodo(id) {
        try {
            const response = await this.patch(`/todos/${id}/toggle`);
            // Validate response data
            if (response.success && response.data) {
                const result = apiTodoSchema.safeParse(response.data);
                if (!result.success) {
    ...
    async bulkUpdateTodos(updates) {
        try {
            ...
            const response = await this.patch('/todos/bulk', { updates: validatedUpdates });
            // Validate response data
            if (response.success && response.data) {
                const validatedData = response.data.map(todo => {
                    const result = apiTodoSchema.safeParse(todo);
    ...
    async searchTodos(query, filters) {
        try {
            ...
            const response = await this.get('/todos/search', { params });
            // Validate response data
            if (response.success && response.data) {
                const validatedData = response.data.map(todo => {
                    const result = apiTodoSchema.safeParse(todo);
...
//# sourceMappingURL=TodoApiClient.js.map