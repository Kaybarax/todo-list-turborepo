...
usage(){ echo "Usage: packages/services/cleanup.sh [--yes] [--dry-run] [--verbose]"; }
...
cd "$(cd "$(dirname "$0")" && pwd)"
echo "Cleaning packages/services artifacts..."
...
if confirm "Remove node_modules in packages/services?"; then run_cmd "rm -rf node_modules"; fi
echo "packages/services cleanup done."
