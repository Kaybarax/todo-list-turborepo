...
StyleDictionary.registerFilter({
  ...
  filter: token => {
    return ['color', 'size', 'space', 'fontFamily', 'fontSize', 'borderRadius', 'boxShadow', 'fontWeight'].includes(
...
StyleDictionary.registerFormat({
  ...
  format: function (dictionary) {
    ...
    const semanticTokens = tokens.filter(token => token.path[0] === 'semantic' && token.path[1] === 'color');
