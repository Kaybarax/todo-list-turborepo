...
const badgeVariants = cva(
  'badge inline-flex items-center gap-1.5 font-semibold transition-all duration-200 transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-offset-1 shadow-sm',
  {
    variants: {
      variant: {
        default: 'badge-neutral shadow-neutral/25 hover:shadow-lg hover:shadow-neutral/40',
        primary: 'badge-primary shadow-primary/25 hover:shadow-lg hover:shadow-primary/40',
        secondary: 'badge-secondary shadow-secondary/20 hover:shadow-md',
        accent: 'badge-accent shadow-accent/25 hover:shadow-lg hover:shadow-accent/40',
        ...
        error: 'badge-error shadow-error/25 hover:shadow-lg hover:shadow-error/40',
      },
      size: {
        xs: 'badge-xs',
        sm: 'badge-sm',
...
const Badge = ({ className, variant = 'default', size, icon, children, ...props }: BadgeProps) => {
  return (
    <span className={cn(badgeVariants({ variant, size }), className)} {...props}>
      {icon && (
        <span className="flex-shrink-0 transition-transform duration-200 group-hover:rotate-12 mr-1">{icon}</span>
      )}
