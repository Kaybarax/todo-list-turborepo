...
usage(){ echo "Usage: packages/config-ts/cleanup.sh [--yes] [--dry-run] [--verbose]"; }
...
cd "$(cd "$(dirname "$0")" && pwd)"
echo "Cleaning packages/config-ts artifacts..."
...
if confirm "Remove node_modules in packages/config-ts?"; then run_cmd "rm -rf node_modules"; fi
echo "packages/config-ts cleanup done."
